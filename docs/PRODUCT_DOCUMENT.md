...
4. **Advise & Fix** — uses Groq (Llama 3.3 70B) to 
   ...
   fix advice, and AI-generated fix content that merchants 
   can review and push directly to Shopify in one click
...
Most diagnostic tools identify problems and stop there. 
AuraScan closes the full loop.

When an issue is found, the merchant can request 
AI-generated fix content, review it, and push it directly 
to their Shopify store in one click — without leaving the 
dashboard or touching Shopify admin manually. The score 
updates in real time after each fix is applied.

**This is not a planned feature. It is working, 
demonstrated, and live on our demo store RiderzPlanet.**

Score journey: 0/100 (broken) → 51/100 (baseline) → 
74/100 (after fixing critical issues) → 100/100 (fully 
optimised).
...
Ravi opens AuraScan and the first thing he sees is a 
number — 51/100. He doesn't need to read anything else 
to know something is wrong.

Below the score, a Groq-generated summary tells him 
exactly why in plain English — 3 of his products have no 
description, his store has no refund policy, and AI agents 
are skipping him because of it.

He clicks into Critical Issues. Each problem is named — 
not vague. "[Riding Jacket] Missing Product Description." 
He clicks it, reads the AI advice, clicks Apply Fix. Groq 
generates a description. He reviews it, confirms it. It 
pushes live to his Shopify store instantly.

His score jumps to 74.

He works through the list. Every fix is one click. Every 
recommendation is specific to his product. By the end, 
his store scores 100/100 — fully readable, fully 
trustworthy, ready to be recommended by AI agents.
...
Track 1 (AI Shopping Agent) was familiar territory — we 
had built something similar for our Infosys project. We 
chose Track 5 because it addresses a deeper, less visible 
problem. Every merchant on Shopify faces it, but none can 
see it. It's not about building another shopping assistant 
— it's about diagnosing the invisible layer that determines 
whether AI agents recommend you or skip you. That's why we 
named it AuraScan — it scans what merchants can't see 
themselves.
...
score than a product with 0 fields filled. Reward-based 
scoring makes improvement visible and motivating. When a 
merchant fixes one issue, they see their score jump 
immediately. That feedback loop is the core of our product 
experience.
...
it, merchant confirms, then it pushes. Control stays with 
the merchant. Friction is reduced, not eliminated.
...
merchant never reads. Advice is fetched on demand — only 
when the merchant explicitly requests it.
...
## 7. What We Chose NOT to Build and Why
...
and variant stock levels. We chose focused depth over 
broad coverage — the checks we built directly impact AI 
agent readiness and are fully supported by the Shopify 
Admin API.
...
within the deadline. The OAuth architecture is understood 
...
auto-fix only where AI-generated content is unambiguous 
and safe to push. Policy pages require human-written 
legal content — auto-generating them would be 
irresponsible.

**Tradeoff 6: Streamlit over custom frontend**
We chose Streamlit over a custom React dashboard. 
Streamlit is less visually polished but allowed us to 
invest time in the analysis pipeline, agentic fix loop, 
and documentation rather than frontend styling. Kasparro's 
own evaluation criteria states the best submissions will 
be the clearest and most thoughtfully built — not the 
most visually elaborate.
